...
首頁標題區會顯示本次網站建置的版本時間，格式為 `YYYY.MM.DD-HH:mm:ss（台灣時間）`，開啟網站即可看到。每次執行 `npm run build` 或 Vercel production deployment 都會自動產生新的版本時間，不需要手動修改版本字串；本機 `npm run dev` 也會顯示啟動時的版本時間。
...
- PTT 會阻擋部分雲端主機；原站回傳 HTTP 403 時，網站會先透過 Jina Reader 無快取中繼取得即時 PTT HTML，只有即時中繼不可用時才改讀 PTTweb 延遲鏡像。
- 匿名即時中繼有流量限制：程式最多分配 8 次列表請求與約 10 次文章請求；看板或頁數太多時會優先保留每個看板最新一頁並顯示提示。
